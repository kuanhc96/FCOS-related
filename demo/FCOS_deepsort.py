...
from demo.FCOS_Evaluator import FCOSEvaluator
...
class VideoTracker(object):
    def __init__(self, cfg, args, video_path, fcos=None):
        ...
        if fcos is not None:
            ...
            fcos = FCOSEvaluator("weights/FCOS_imprv_R_50_FPN_1x.pth",config="./configs/fcos/fcos_imprv_R_50_FPN_1x.yaml")
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            #print(im.shape)
            #im = imutils.resize(ori_im, height=800)
            #im = cv2.cvtColor(ori_im, cv2.COLOR_BGR2RGB)

            # do detection
            bbox, cls_conf = self.detector.load_detections_and_confidence(im)
            #print(bbox)
            bbox_xywh = np.zeros((len(bbox), 4))
            #bbox_xywh[:, 0] = bbox[:, 2]
            #bbox_xywh[:, 1] = bbox[:, 3]
            #bbox_xywh[:, 2] = bbox[:, 2] - bbox[:, 0]
            #bbox_xywh[:, 3] = bbox[:, 3] - bbox[:, 1]
            for i in range(len(bbox)):
                # current = [bbox[i][0][0], bbox[i][0][1], bbox[i][1][0] - bbox[i][0][0], bbox[i][1][1] - bbox[i][0][1]]
...
def parse_args():
    ...
    #parser.add_argument("--config_detection", type=str, default="./configs/fcos/fcos_imprv_R_50_FPN_1x.yaml")
